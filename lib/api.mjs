...
// Timed publishing: a quiz is visible to students while active and its
// publishedUntil timestamp (ISO string) is absent or still in the future.
const validPublishedUntil = value => value === null || value === undefined ||
  (typeof value === 'string' && value.length <= 40 && !Number.isNaN(Date.parse(value)));
const isPublished = (quiz, now = Date.now()) => quiz && quiz.active !== false &&
  (quiz.publishedUntil == null || Date.parse(quiz.publishedUntil) > now);
// Student avatar faces served from /assets/avatar-NN.png (24 files, see public/assets).
const validAvatar = value => typeof value === 'string' && /^assets\/avatar-(0[1-9]|1[0-9]|2[0-4])\.png$/.test(value);
    ...
    try {
      ...
      if (route[0] === 'quizzes') {
        if (method === 'GET' && !route[1]) {
          ...
          return json(isAdmin ? data.quizzes : data.quizzes.filter(q => isPublished(q)).map(q => ({
            id: q.id, quizNumber: q.quizNumber, title: q.title, instructions: q.instructions,
            active: true, publishedUntil: q.publishedUntil ?? null,
            vocabulary: q.vocabulary.map(({ word }) => ({ word }))
          })));
        }
        if (method === 'POST' && !route[1]) {
          ...
          if (!validPublishedUntil(quiz.publishedUntil)) fail(400, 'מועד פרסום לא תקין.');
          const fields = { title: quiz.title.trim(), instructions: (quiz.instructions || '').slice(0, 2000),
            active: quiz.active !== false,
            publishedUntil: quiz.publishedUntil == null ? null : new Date(Date.parse(quiz.publishedUntil)).toISOString(),
            vocabulary: quiz.vocabulary.map(v => ({ word: v.word.trim(), answers: v.answers.map(a => a.trim()) })) };
        ...
        if (method === 'POST' && !route[1]) {
          ...
          if (nameParts.length < 2 || studentName.length < 3 || studentName.length > 60 || /[,;]/.test(studentName)) {
            fail(400, 'יש למלא שם מלא אחד (שם פרטי ושם משפחה).');
          }
          const { data } = await catalog();
          const quiz = data.quizzes.find(q => q.id === input.quizId && isPublished(q));
          if (!quiz) fail(404, 'הבוחן אינו זמין להגשה. ייתכן שהמורה הסתירה אותו או שתוקף הפרסום פג.');
          ...
          const key = `submissions/sub-${requestId}`;
          // Only one submission per full name per quiz: reject a second full name entry.
          // The idempotent retry with the same requestId key is excluded from this check.
          const { blobs: existingBlobs } = await store.list({ prefix: 'submissions/' });
          if (existingBlobs.length) {
            const existing = await Promise.all(existingBlobs
              .filter(b => b.key !== key)
              .map(b => store.get(b.key, { type: 'json' }).catch(() => null)));
            const duplicate = existing.some(s => s && s.quizId === quiz.id && normalize(s.studentName) === normalize(studentName));
            if (duplicate) fail(409, 'שם מלא זה כבר הגיש את הבוחן. כל תלמיד/ה יכול/ה להגיש פעם אחת בלבד.');
          }
          const submission = { id: `sub-${requestId}`, studentName, quizId: quiz.id, quizTitle: quiz.title,
            avatar: validAvatar(input.avatar) ? input.avatar : undefined,
